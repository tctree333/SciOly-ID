...
class Score(commands.Cog):
    ...
    @staticmethod
    async def user_lb(ctx: commands.Context, title, page, database_key=None, data=None):
    ...
    async def score(
        ...
        scope: Literal["total", "server", "channel", "t", "s", "c"] = "channel",
    ):
    ...
    async def userscore(
        ...
        *,
        ...
        if user is not None:
            ...
            score = database.zscore("users:global", str(ctx.author.id))
            if score is not None:
    ...
    async def missed(
        ...
        await send_leaderboard(
            ctx,
            f"Top Missed {config.options['id_type'].title()} ({scope})",
            page,
            database_key,
            data,
        )
